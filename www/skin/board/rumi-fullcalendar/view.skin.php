...
include_once(FC_PLUGIN_PATH.'/function.lib.php'); // 공통 함수 모음
...
$ifDelete = ($delete_href) ? true : false;
$ifUpdate = ($update_href) ? true : false;

// 이전글
$prev_href = $board_skin_url."/view.php?bo_table=".$bo_table."&wr_id=".$prev['wr_id'];

// 다음글
$next_href = $board_skin_url."/view.php?bo_table=".$bo_table."&wr_id=".$next['wr_id'];
?>

<script>
var ifDelete = "<?php echo $ifDelete; ?>";
var ifUpdate = "<?php echo $ifUpdate; ?>";
var cmt_no = "<?php echo $cmt; ?>";
$(function() {

    if(cmt_no && cmt_no > 0) {
        setTimeout(function() {
            var cmt_offset = $("#c_"+cmt_no).offset();
            console.log(cmt_offset);
            $('html').animate({
                scrollTop : cmt_offset.top
            }, 400);
        },100);
    }

    $("div.rumiButton button:contains('저장')", parent.document).hide();
    $("div.rumiButton button:contains('취소')", parent.document).hide();

    if(ifUpdate) {
        $("div.rumiButton button:contains('수정')", parent.document).show();
    }
    else {
    ...
    if(ifDelete) {
        $("div.rumiButton button:contains('삭제')", parent.document).show();
    }
    else {
    ...
    <section id="bo_v_info">
        ...
        <div class="scdate-txt">
            ...
            if ($view['wr_1'] == $view['wr_2']) {
                ...
                echo '<span class="date-txt">'.wz_get_hangul_date($view['wr_1']).' 부터 '.wz_get_hangul_date($view['wr_2']).' 까지</span>';
            }
            ?>
            <span class="date-title">기간</span>
        </div>
	        ...
	        <ul class="btn_bo_user bo_v_com">
	        	...
	        	<li>
		        	...
		        	<ul class="more_opt is_view_btn">
			            <?php if ($update_href) { ?><li><a href="<?php echo $update_href ?>" onclick="location.href='<?php echo $update_href ?>'"id="btn_edit">수정<i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></li><?php } ?>
			            <?php if ($delete_href) { ?><li><a href="<?php echo $delete_href ?>" id="delete" onclick="del(this.href); return false;">삭제<i class="fa fa-trash-o" aria-hidden="true"></i></a></li><?php } ?>
